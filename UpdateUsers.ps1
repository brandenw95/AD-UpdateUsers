...
function singleUpdate{
    ...
    if ($matchedEmployee) {
        ...
        #New user info - May 10th 2024 BW
        $UserCity = $matchedEmployee.location
        $UserExtension = $matchedEmployee.workPhoneExtension
        $UserFaxNumber = "250-652-4427"

        if($UserExtension -eq $null){
            $UserExtension = " "
        }
        ...
        if($responseConfirmation -eq "Y"){
            ...
            if($debug -eq "False"){
                ...
                if($null -eq $ValidActiveDirectoryUser -or $null -eq $ValidActiveDirectoryManager){
                    ...
                    Set-ADUser -Identity $userSamNameInBamboo -Title $jobTitle -OfficePhone $workPhone -Company $company -Department $department -Fax $UserFaxNumber -Replace @{msExchExtensionCustomAttribute1=$UserExtension} 
                    }
                    ...
                    Write-Output "Company: $company"
                    Write-Output "Reports To: $supervisorName"
                    Write-Output "Fax line: $UserFaxNumber"
                    Write-Output "User Extension: $UserExtension"
                    Write-Output "User Location: $UserCity"
                    Write-Host "Status: " -NoNewline
                    Write-Host "SUCCESS" -ForegroundColor Green
                    Write-Output "==============================="
                    #Set User in AD
                    if($null -eq $UserExtension -or $UserExtension -eq " "){
                    $UserExtension = ""
                    }
                    #Set-ADUser -Identity $userSamNameInBamboo -Title $jobTitle -Manager $supervisorSam -OfficePhone $workPhone -Company $company -Department $department -Fax $UserFaxNumber -Replace @{msExchExtensionCustomAttribute1=$UserExtension} 
                    Set-ADUser -Identity $userSamNameInBamboo -Title $jobTitle -Manager $supervisorSam -OfficePhone $workPhone -Company $company -Department $department -Fax $UserFaxNumber
                }
            }
            else{
    ...
    if($debug -eq "False"){
        ...
        foreach($user in $Users){
            ...
            if(($SingleUserEmailAddress -ne " ") -and ($SingleUserEmailAddress -match "@seafirstinsurance.com")){
                ...
                if($SingleUserGivenName -notin "Jody", "Douglas", "Doug", "Ana", "Randi", "Jaime", "Ali"){
